...
$string['browserstorage'] = 'ブラウザストレージ';
...
$string['jsmaxstoreditems'] = 'ブラウザのセッションストレージ内の最大コンテンツアイテム';
$string['jsmaxstoreditems_desc'] = 'ユーザーがタイルのメイン画面でブラウジングしているとき、ブラウザは、タイルの概要画面自体のHTMLと、各タイルのコンテンツを、ここで選択した最大数まで保存します。これにより、新しいタイルがクリックされたときに非常に速いレスポンスが可能になります。この値を大きくしすぎると、ブラウザのストレージがいっぱいになってしまう可能性があります（ただし、これまでのテストでは問題ありませんでした）。';
...
$string['jsstoredcontentdeletemins'] = 'セッション中に保存されたコンテンツは（分）後に削除される';
$string['jsstoredcontentdeletemins_desc'] = 'タイルをクリックするたびに、現在のHTMLのためのスペースを確保するために、ユーザーのブラウザはクリーンアップを実行し、セッションストレージから古い保存内容をすべて削除します（信頼性がなくなり、いずれにしてもサーバーから新しいコピーが必要になるという理由から）。';
$string['jsstoredcontentexpirysecs'] = 'セッション中に保存されたコンテンツの有効期限（秒）';
$string['jsstoredcontentexpirysecs_desc'] = 'ユーザがタイルをクリックしたとき、ブラウザのセッションストレージ内のタイルコンテンツがこれより古い場合、タイルはまだ表示されますが、新しいコピーを交換するためにサーバへのバックグラウンドリクエストが行われます。これは、ブラウザがそのコピーを保存した後に、コースの更新を可能にするためです。保存されているコンテンツがこれよりも新しい場合、最新のものとみなされ、サーバーへのリクエストを行わずに表示されます。';
